...
function uvOctagonalPyramid(radius, height, noBottom) {
    radius = radius || 0.5;
    height = height || 2 * radius;
    var slices = 8;
    var vertexCount = slices + 1;
    if (!noBottom) {
        vertexCount += slices + 1;
    }
    var triangleCount = slices;
    if (!noBottom) {
        triangleCount += slices;
    }
    ...
    for (var i = 0; i < slices; i++) {
        ...
        vertices[kv++] = -height / 2;
        var normallength = Math.sqrt(height * height + radius * radius);
        normals[kv - 3] = (c * height) / normallength;
        normals[kv - 2] = (s * height) / normallength;
        normals[kv - 1] = (radius) / normallength;
        texCoords[kt++] = 0.5 + 0.5 * c;
        texCoords[kt++] = 0.5 + 0.5 * s;
    }

    // Indeksy dla ścian bocznych
    for (var i = 0; i < slices; i++) {
        indices[ki++] = 0;
        indices[ki++] = i + 1;
        indices[ki++] = (i + 1) % slices + 1;
    }

    if (!noBottom) {
        var baseCenterIndex = kv / 3;
        vertices[kv++] = 0;
        vertices[kv++] = 0;
        vertices[kv++] = -height / 2;
        normals[kv - 3] = 0;
        ...
        for (var i = 0; i < slices; i++) {
            indices[ki++] = baseCenterIndex;
            indices[ki++] = baseCenterIndex + i + 1;
            indices[ki++] = baseCenterIndex + ((i + 1) % slices) + 1;
        }
    }
